...
class STPServer(STPServerBase):
    ...
    def route(self, namespace:str):
        ...
        def inner(func:callable):
            ...
            if num_args != 1:
                InvalidImplementation("Route function for '{}' accepts excatly 1 arg. {} were given!".
                        format(namespace, num_args))

            def namespace_callback(body:dict, header:dict, sender_id:str, protocol:str):
                packet = Packet(
                    data=body, headers=PacketHeader(**header), sender=sender_id, protocol=protocol)
                func(packet)
                
            self.add_callback(namespace=namespace, callback=namespace_callback)

            def modified(*args):
                raise UsageError("The Route function is not intented to be called outside!") 
            return modified
    ...
    def on_peer_list_update(self, func:callable):
        ...
        num_args = len(inspect.signature(func).parameters)
        if num_args != 2:
            InvalidImplementation("peer_list_update event handler accepts exactly 2 arg. {} were given!".
                                  format(num_args))
    ...
    def __bind_private_callbacks(self):
        ...
        def peer_check(body:dict, header:dict, sender_id:str, protocol:str):
            ...
            if sender_id not in self._peers: 
                ...
                [callback(new_peer=peer, removed_peers=[]) for callback in self._peer_list_update_callbacks]
        ...
        def peer_join(body:dict, header:dict, sender_id:str, protcol:str):
            if protcol.upper() == "UDP": 
                ...
                self.send_tcp("iamheredude", to_addr=sender_id, to_port=header['tcpport'],
                            namespace="/peer-join", enc_key=False, pass_pub_key=True) 
    ...
    async def cleanup_peers(self):
        ...
        while True:
            ...
            if removed_peers:
                for key in removed_peers.keys(): del self._peers[key]
                [callback(new_peer=None, removed_peers=removed_peers.values()) for callback in self._peer_list_update_callbacks]
            removed_peers = {}
    ...
    def request_sync(self):
        """Request other peers to send their address"""
        self.send_udp("gimmeurnumberdude", "/peer-join", enc_key=False, pass_pub_key=True)

    def send_to_peer(self, namespace:str, data, peer_ip:str, encrypt:bool=True) -> bool:
        ...
        return self.send_tcp(data, namespace=namespace,
                to_addr=peer_ip, to_port=peer.tcpport, 
                enc_key=peer.public_key, pass_pub_key=True)
